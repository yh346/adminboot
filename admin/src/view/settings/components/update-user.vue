  ...
  >
    <Form ref="addForm" :model="user" :label-width="100" :rules="rules">
      <Row :gutter="32">
        ...
        <Col span="12">
          <FormItem label="登录密码" label-for="user_pwd">
            <Input element-id="user_pwd" type="text" v-model="user.user_pwd" placeholder="如需更改，请输入"></Input>
          </FormItem>
        </Col>
      </Row>
      <Row :gutter="32">
...
<script>
import { update, getInfo } from "@/api/admin-user";
  ...
  methods: {
    getUserInfo() {
      ...
      getInfo(this.user.admin_id).then(res => {
        ...
        if (!res.code) {
          this.user = res.data;
          this.user.is_status_ok = !this.user.is_disabled;
          this.user.dept_id = this.user.dept_id.toString();
          this.user.sex = this.user.gender ? 1 : 0;
        } else {
    ...
    handleOk(name) {
      this.$refs[name].validate(async valid => {
        if (valid) {
          ...
          var res = await update(this.user);
