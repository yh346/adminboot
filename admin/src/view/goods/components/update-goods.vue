<template>
  ...
  >
    <Form ref="updateForm" :model="goods" :label-width="100" :rules="rules">
      <Collapse v-model="collapse_value" accordion>
        ...
        <Panel name="6">详细信息
          <p slot="content">
            <ueditor
              :margin="'-10px'"
              :html="goods.description"
              @on-sync="(html)=>goods.description=html"
            ></ueditor>
          </p>
        </Panel>
      </Collapse>
...
import { info, update } from "@/api/goods";
...
export default {
  ...
  methods: {
    ...
    getInfo() {
      this.spinShow = true;
      info(this.goods.goods_id).then(res => {
        this.spinShow = false;
        if (!res.code) {
          this.goods = res.data;
          this.goods.goods_type_id = this.goods.goods_type_id.toString();
          this.goods.brand_id = this.goods.brand_id.toString();
          this.goods.type_id = this.goods.type_id.toString();
          this.image_array = deepCopy(this.goods._image_array);
          this.goods.image_array = this.goods._image_array.map(p => p.url);
          listBrands(this.goods.goods_type_id).then(res => {
            if (!res.code) {
              this.goods_brand_array = res.data;
            }
          });
        }
      });
    ...
    handleOk(name) {
      this.$refs[name].validate(async valid => {
        if (valid) {
          ...
          var res = await update(this.goods);
